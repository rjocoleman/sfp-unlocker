#!/bin/sh
# Build the iPXE network-boot artefacts.
# Usage: image/build-pxe.sh [OUTDIR]   (default: dist/pxe)
#
# Fetches Alpine's netboot kernel/initramfs/modloop, builds the apkovl, and
# writes a filled-in sfp.ipxe. Serve OUTDIR over HTTP(S) or attach it to a
# release, then point iPXE at sfp.ipxe.
